...
class Model {
  ...
  reset() {
    ...
    const diffLabel = diffLabels[this.currentDifficulty]
    ...
    const stateBefore = [...this.buttons];

    // Nouvel état de départ = état initial du test (tout éteint par défaut).
    const initial = (this.rule?.initialState || Array(9).fill(0)).map(v => !!v);
  ...
  // Maintenant async !
  async setTest(testNumber, difficulty = 'easy') {
  ...
  exportJSON() {
    ...
    addValidationQuestionResult(questionIndex, isCorrect) {
        ...
        this.validationAllCorrect = isCorrect && this.validationAllCorrect;
